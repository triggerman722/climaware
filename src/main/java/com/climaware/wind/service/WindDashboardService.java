...
public class WindDashboardService {
    ...
    public WindDashboard getDashboard() {
        ...
        List<Object[]> ppAll = SystemDataAccess.getAll("select s.latitude, s.longitude, count(s.id), min(s.year), max(s.year), min(s.windspeed), max(s.windspeed), avg(s.windspeed), s.stationid " +
                " from WindRecord s " +
                "GROUP BY s.stationid, s.latitude, s.longitude");
